...
deklaracio:
    UNSIGNED ID ENDL
    {
        ...
        if( szimbolumtabla.count(*$2) > 0 )
        {
            ...
            << "Korabbi deklaracio sora: " << szimbolumtabla[*$2].def_sora << std::endl;
            error( ss.str().c_str() );
        }
        szimbolumtabla[*$2] = var_data(d_loc__.first_line,Egesz);
        delete $2;
    }
|
    BOOL ID ENDL
    {
        ...
        szimbolumtabla[*$2] = var_data(d_loc__.first_line,Logikai);
        delete $2;
    }
;
...
ertekadas:
    ID EQV kifejezes ENDL
    {
        ...
        if( szimbolumtabla[*$1].tipus != $3->ktip)// $3->ktip )
        {
            std::stringstream ss;
...
|
    ID
    {
        ...
        var_data vl = szimbolumtabla[*$1];
        $$ = new kifejezes_leiro( vl.def_sora, vl.tipus );
    }
|
...
|
	OPENPAR kifejezes KKOPKJ kifejezes KKOPKP kifejezes CLOSEPAR
	{
		if ($2->ktip != Logikai)
		{
			std::stringstream ss;
            ss << $2->sor << ": A KKOP-ban a kezdeti kifejezes helyen nem logikai tipusu kifejezes all." << std::endl;
            error( ss.str().c_str() );
		}
		if ($4->ktip != $6->ktip)
		{
			std::stringstream ss;
            ss << $4->sor << ": A KKOP-ban az eredmeny kifejezesek tipusa nem azonos." << std::endl;
            ss << $6->sor << ": A KKOP-ban az eredmeny kifejezesek tipusa nem azonos." << std::endl;
            error( ss.str().c_str() );
		}
