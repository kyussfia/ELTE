...
namespace LightDuel
{
    ...
    public class UnitTest1
    {
        ...
        private void startGameTester(int size)
        {
            ...
            Assert.IsTrue(this.model.Blue.type == Model.Players.Blue);
            Assert.AreEqual((size / 2), this.model.Red.row);
            Assert.AreEqual(size - 1, this.model.Red.col);
            Assert.AreEqual(4, this.model.Red.dir);
            Assert.IsTrue(this.model.Red.type == Model.Players.Red);

            Assert.IsTrue(this.model.fields[0][0] == Model.Players.No);
        }
        ...
        private void tickedTest(object sender, PlayerMoveEventArgs e)
        {
            ...
            Assert.IsTrue(this.model.fields[e.BlueX][e.BlueY] == Model.Players.Blue);
            Assert.IsTrue(this.model.fields[e.RedX][e.RedY] == Model.Players.Red);
            Assert.IsTrue(this.model.isTimerWorks());
        }
        ...
        public void playerWonTester(Model.Player winner, int gameSize)
        {
            for (int i = 0; i < gameSize; i++)
            {
                if (i == gameSize - 1)
            ...
            if (winner.type == Model.Players.Blue)
            {
